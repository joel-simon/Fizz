...
function is(obj, req) {
  try {
    ...
    console.log('VAR FAILED TYPE CHECK')
    console.log('\tVAR:'+JSON.stringify(obj));
    console.log('\tTYPE:'+JSON.stringify(req));
    throw(e);
  } 
}
...
function IS(obj, req) {
  ...
  switch (typeof req) {
    ...
    case 'function':
      if (!req(obj)) throw (JSON.stringify(obj)+' failed '+req)
      break;  
    ...
    case 'string':
      ...
      // default types
        ...
        if (! types[req](obj)) {
          throw JSON.stringify(obj)+' should be a ' + req;
        }
      }
      // arrays : "[type]"
        ...
        if (!Array.isArray(obj))
          throw JSON.stringify(obj)+' should be an array.';
        if (obj.length === 0) {
          return;
        }
    ...
    case 'object':
      if (typeof obj !== 'object') throw JSON.stringify(obj)+' should be an object';
      if (obj === null) return;
      // {} case
      for(var e in req) {
        if (!obj.hasOwnProperty(e)) throw JSON.stringify(obj)+' does not contain field \''+e+'\' for requirement '+JSON.stringify(req)+ typeof(req);
        is(obj[e], req[e]);
      }
